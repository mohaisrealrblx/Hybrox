...
module.exports = {
  ...
  execute: async (message, args, client) => {
    ...
    if (!message.member.permissions.has('KICK_MEMBERS')) { return message.channel.send("> <a:cross:805828442247528468> ;; You're not allowed to use this command.").then(m => m.delete({ timeout: 10000 }))}

    if (message.guild.members.resolve(member.id)) { 
    if (message.member.roles.highest.comparePositionTo(member.roles.highest) <= 0) { return message.channel.send("> <a:cross:805828442247528468> ;; You may not kick a user with a higher or equal hierarchy level than you.").then(m => m.delete({ timeout: 10000 }))}

    if (!member.bannable) { return message.channel.send("> <a:cross:805828442247528468> ;; I can't kick that user. :(").then(m => m.delete({ timeout: 10000 }))}

    let reason = args.slice(1).join(" ") ? args.slice(1).join(" ") : `Reason unspecified.`
    message.guild.member(member.id).kick({ reason: reason })

    embed.setThumbnail(!!member.user ? member.user.displayAvatarURL() : member.displayAvatarURL())
    embed.setDescription(`**__Kick Protocol.__**\n\n> **User:** ${member.user.username}\n> **Reason:** ${reason}\n> **Moderator:** ${message.author.username}\n\n> **BAN LOGGED INTO THE DATABASE.**`)
    ...
    trello.addCard('Discord Moderation Log', `**__Kick Protocol.__**\n\n> **User:** ${member.user.username}\n> **Reason:** ${reason}\n> **Moderator:** ${message.author.username}\n\n> **BAN LOGGED INTO THE DATABASE.**`, "61707123a1e1354f23f8b2af",
